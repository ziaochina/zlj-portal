...
class reducer {
    ...
    load = (state, { menu }) => {
        ...
        const childApp = history.getChildApp('portal')
        if (childApp)
    ...
    setContent = (state, name, appName, appProps) => {
        ...
        var hitIndex = openTabs.findIndex(o => o.get('name') == name || o.get('appName') == appName)
        var hit = hitIndex != -1
        ...
        if (!hit) {
            ...
            if (!isTabsStyle) {
                ...
                state = this.metaReducer.sf(state, 'data.openTabs.' + hitIndex, content)
            }
        }

        setTimeout(() => {
            history.pushChildApp('portal', content.get('appName'))
        }, 0)
